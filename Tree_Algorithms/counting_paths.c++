...
int  dfscall(int node=1,int p=1)
{
	for(int &i:edge[node])
	{
		if(i==p) continue;
		val[node]+=dfscall(i,node);
	}
	return val[node];
}
...
int main()
{
	...
	val=depth=tin=tout=vector<int> (n+1,0);
	...
	while(q--)
	{
		int a,b;cin>>a>>b;
		int lca=findlca(a,b);
		if(lca>1) {val[a]++;val[b]++;val[lca]--;val[lift[lca][0]]--;}
		else 	  {val[a]++;val[b]++;val[lca]--;}
 
	}
 
 
	dfscall();
	forr(i,1,n+1) cout<<val[i]<<" ";cout<<"\n";
