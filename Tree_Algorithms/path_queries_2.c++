...
lli segquery(int l,int r)
{
	...
	while(l<=r)
	{
		if(l&1) 	maxi=max(maxi,seg[l++]);
		if(!(r&1))  maxi=max(maxi,seg[r--]);
		l=l>>1;r=r>>1;
	}
	return maxi;
}
...
lli query(int l,int r)
{
	...
	while(head[l]!=head[r])
	{
		...
		{
			maxi=max(maxi,segquery(pos[head[r]],pos[r]));
			r=par[head[r]];
		}
	}
	...
	if(head[l]==head[r])
	{
		if(pos[l]>pos[r])
		{
			maxi=max(maxi,segquery(pos[r],pos[l]));
		}
		else
		{
			maxi=max(maxi,segquery(pos[l],pos[r]));
		}
	}
...
// simple application of HLD O(log^2n) 
// wondering how  O(logn) can be achieved
